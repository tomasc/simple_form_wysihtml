class WysihtmlInput < SimpleForm::Inputs::Base
  ...
  # @return [String]
  def input
    template.content_tag :div, :class => %w(wysihtml) do
      toolbar + text_area
    end
  end
  ...
  # @return [Array]
  def command_list
    input_options[:commands] || Settings.wysihtml.default_commands.map(&:to_hash)
  end
